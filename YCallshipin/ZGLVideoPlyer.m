...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.videoMaskView = [[ZGLVideoMaskView alloc]initWithFrame:frame playBtnClick:^(UIButton *playBtn) {
            [weakself playBtnClick:playBtn];
        } fullScreenBtnClick:^(UIButton *fullScreenBtn) {
             [weakself fullScreenClick:fullScreenBtn];
        } kuaijinBtnClick:^(UIButton *kuaijin) {
             [weakself kuaijinClick:kuaijin];
        } houtuiBtnClick:^(UIButton *houtui) {
             [weakself houtuiClick:houtui];
        } backClick:^(UIButton *backBtn) {
           //  [weakself fullScreenClick:backBtn];
        }];
...
-(void)kuaijinClick: (UIButton *)btn {
    NSLog(@"快进");
    [self play];

    //让视频从指定的CMTime对象处播放。
    NSLog(@"%lld,\n %d", self.playerItem.currentTime.value, self.playerItem.currentTime.timescale);
    CMTime startTime = CMTimeMakeWithSeconds(self.playerItem.currentTime.value/self.playerItem.currentTime.timescale-1, self.playerItem.currentTime.timescale);
     [self.playerItem seekToTime:startTime toleranceBefore:CMTimeMake(1, self.playerItem.currentTime.timescale) toleranceAfter:CMTimeMake(1, self.playerItem.currentTime.timescale)];
        [self pause];
        self.videoMaskView.bottomBackgroundView.hidden = NO;


}
-(void)houtuiClick: (UIButton *)btn {
    NSLog(@"后推");
    [self play];
...
- (void)setVideoUrlStr:(NSString *)videoUrlStr {
    ...
    if ([NSURL fileURLWithPath:videoUrlStr]) {
        self.playerItem = [AVPlayerItem playerItemWithURL:[NSURL fileURLWithPath:videoUrlStr]];
