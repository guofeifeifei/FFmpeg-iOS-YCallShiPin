...
//    int64_t targetFrame = (int64_t)((double)timeBase.den / timeBase.num  *  seconds );
    int64_t targetFrame = seconds;

    NSLog(@"%lld",targetFrame );
...
- (void)seekTime2:(double)seconds {
    AVRational timeBase = XYQFormatCtx->streams[videoStream]->time_base;
    int64_t targetFrame = (int64_t)((double)timeBase.den / timeBase.num * seconds);
//    int64_t targetFrame = seconds;

    av_seek_frame(XYQFormatCtx,
                       videoStream,
                       targetFrame,
                       AVSEEK_FLAG_ANY);
    avcodec_flush_buffers(XYQCodecCtx);
...
- (BOOL)stepFrame {
    int frameFinished = 0;
//    XYQCodecCtx->time_base.den = 121212;
//    XYQCodecCtx->time_base.num = 1;

    while (!frameFinished && av_read_frame(XYQFormatCtx, &packet) >= 0) {
//        NSLog(@"pkt.size=%d,pkt.pts=%lld, pkt.data=0x%x", packet.size, packet.pts,(unsigned int)packet.data);
...
- (double)zhenCount{
    return  XYQCodecCtx->pkt_timebase.den;
}
- (double)currentTime {
    AVRational timeBase = XYQFormatCtx->streams[videoStream]->time_base;
